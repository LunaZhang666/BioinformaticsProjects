...
f.close()
t = '' # fna/faa
name = re.match(r'^.*[\.]',sys.argv[1])
if infile[0].startswith("@"): # fastq
    for c in infile[1]: # check first sequence t=faa once hit a character that isn't ATCG 
        if c in 'ATCGNatcgn\n':
            t = 'fna'
        else:
            t = 'faa' 
    ...
    for line in range(0,len(infile),4):
        ...
        outfile.write(infile[line+1]) # keep sequence as-is
